...
class CDbAuthManager extends CAuthManager
{
	...
	public $atuhGroupTable	= 'atuhGroupTable';
	public $userGroupTable	= 'userGroupTable';
	public $userGetGroupTable	= 'userGetGroupTable';
	/**
	...
	{
		//如果为基于用户组的验证，跳至用户组验证函数
		if(!empty($this->groupAuth))
			return $this->getGroupAuthAssignments($userId);
		$rows=$this->db->createCommand()
			->select()
			->from($this->assignmentTable)
	...
	public function getGroupAuthAssignments($userId)
	{
		$assignments	= array();
		if(Yii::app()->user->isGuest)
			return $assignments;
		//首先获取用户组ID
		if(empty(Yii::app()->user->groupid))
		{
			$groupID	= $this->userGetGroupTable['groupid'];

			$group=$this->db->createCommand()
				->select($groupID)
				->from($this->userGetGroupTable['tableName'])
				->where($this->userGetGroupTable['userid'].'='.$userId)
				->queryRow();
			if(empty($group))
				return $assignments;
		...
		//获取用户组-角色信息
		$authGroups =$this->db->createCommand()
			//	->select($this->atuhGroupTable['role'])
				->from($this->atuhGroupTable['tableName'])
				->where($this->atuhGroupTable['groupid'].'='.Yii::app()->user->groupid)
				->queryAll();
//		echo "<pre>";print_r($authGroups);exit;
		foreach ($authGroups as $authGroup)
			$assignments[$authGroup['role']]	=
			new CAuthAssignment($this,$authGroup['role'],$userId,NULL,NULL);
